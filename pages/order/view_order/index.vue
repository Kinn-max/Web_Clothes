...
// ─── Thanh toán (Banking) ─────────────────────────────────────────────────────
// Nếu dự án có VNPay hoặc cổng thanh toán khác, gọi ở đây.
// Hiện tại chỉ cập nhật paymentStatus = "paid" trực tiếp (demo).
...
const handlePayNow = async (order: Order) => {
  ...
  try {
    ...
    // Ví dụ: await createVnpayPayment({ orderId: order.id })
