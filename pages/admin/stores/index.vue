...
import type { Store, StoreUpdate } from "@/types/store";

definePageMeta({ layout: "admin" });

const { useGetAllStores, useUpdateStore } = useStore();

// ── Fetch tất cả stores ───────────────────────────────────
const { data: storesData, isLoading } = useGetAllStores()
const myStores = computed<Store[]>(() => storesData.value ?? [])
...
const handleUpdateStore = () => {
  ...
  const { id, ...rest } = selectedStore.value
  const payload: StoreUpdate = {
    name:           rest.name,
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8">
    ...
    <!-- Empty -->
    <div v-else-if="myStores.length === 0"
      ...
      <h3 class="text-xl font-semibold text-gray-900 mb-2">Chưa có cửa hàng nào</h3>
      ...
      <div v-for="store in myStores" :key="store.id"
        ...
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <!-- Địa chỉ -->
            <div class="flex items-start gap-3 p-4 rounded-xl bg-gradient-to-br from-red-50 to-red-100 border border-red-200">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-red-700 uppercase mb-1 tracking-wide">Địa chỉ</p>
            ...
            <div v-if="store.phone"
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-green-700 uppercase mb-1 tracking-wide">Điện thoại</p>
                <p class="text-sm text-gray-900 font-medium">{{ store.phone }}</p>
              </div>
            </div>
            ...
            <div v-if="store.email"
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-blue-700 uppercase mb-1 tracking-wide">Email</p>
                <p class="text-sm text-gray-900 font-medium break-all">{{ store.email }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="w-full">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h4 class="font-bold text-gray-900 text-lg">Vị trí trên bản đồ</h4>
            </div>
            <div class="w-full h-[450px] rounded-xl overflow-hidden border-2 border-gray-300 shadow-xl">
              <iframe v-if="store.latitude && store.longitude" class="w-full h-full" loading="lazy"
              ...
              <div v-else class="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                ...
                <p class="text-lg font-semibold text-gray-600 mb-1">Chưa có tọa độ bản đồ</p>
                <p class="text-sm text-gray-500">Vui lòng cập nhật vị trí cửa hàng</p>
              </div>
            </div>
          </div>
        </div>
  ...
  <Teleport to="body">
    <Transition enter-active-class="transition-opacity duration-300" leave-active-class="transition-opacity duration-300"
      ...
      <div v-if="showEditModal && selectedStore"
        ...
        <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-xl font-bold text-white">Cập nhật cửa hàng</h2>
                <p class="text-blue-100 text-sm">Chỉnh sửa thông tin cửa hàng</p>
              </div>
            </div>
          ...
          <!-- Modal body -->
          <div class="p-6 max-h-[calc(100vh-200px)] overflow-y-auto space-y-5">
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                Tên cửa hàng <span class="text-red-500">*</span>
              </label>
              <input v-model="selectedStore.name" placeholder="Nhập tên cửa hàng"
            ...
            <div>
              ...
              <textarea v-model="selectedStore.address_detail" placeholder="Nhập địa chỉ chi tiết" rows="3"
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Tọa độ bản đồ</label>
              <div class="grid grid-cols-2 gap-4">
                <input v-model="selectedStore.latitude" placeholder="Latitude (VD: 10.7769)" type="number" step="any"
                ...
                <input v-model="selectedStore.longitude" placeholder="Longitude (VD: 106.7009)" type="number" step="any"
                  class="border-2 border-gray-300 rounded-lg px-4 py-3 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="selectedStore.phone" placeholder="Nhập số điện thoại" type="tel"
                  class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all" />
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Email</label>
                <input v-model="selectedStore.email" placeholder="Nhập email" type="email"
                  class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all" />
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-200">
            <button @click="closeEdit" :disabled="isUpdating"
            ...
            <button @click="handleUpdateStore" :disabled="isUpdating"
              class="px-6 py-2.5 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition-all disabled:opacity-50 flex items-center gap-2">
              <svg v-if="isUpdating" class="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
