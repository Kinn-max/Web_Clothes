<template>
  ...
  <div
    v-if="open"
    class="fixed bottom-6 right-6 w-[380px] h-[600px] max-h-[calc(100vh-120px)] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden z-50 border border-gray-100 transition-all animate-in"
  >
    <!-- Header -->
    <div class="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-glow-primary-600 to-glow-primary-400 text-white relative overflow-hidden">
      <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-3xl -mr-16 -mt-16" />
      ...
      <div class="flex items-center gap-1 relative z-10">
        ...
        <NuxtLink
          to="/chatbot"
          class="w-8 h-8 rounded-full bg-black/10 flex items-center justify-center hover:bg-black/20 transition-colors"
          title="Mở trang chat"
          @click="open = false"
        >
    ...
    <div
      ...
      class="flex-1 overflow-y-auto px-5 py-5 space-y-5 bg-[#FFFCFC] no-scrollbar"
    >
      <div v-for="(msg, index) in messages" :key="index" class="space-y-3">
        ...
        >
          <div
            class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs border border-gray-100"
          ...
          <div
            ...
            :class="msg.role === 'user'
              ...
              : 'bg-white text-gray-800 rounded-2xl rounded-bl-none border border-gray-100 shadow-sm'"
          >
            {{ msg.content }}
          </div>
        </div>
      ...
      <!-- Typing -->
      <div v-if="isPending" class="flex items-end gap-3">
    ...
    <div class="p-4 bg-white border-t border-gray-50 flex flex-col gap-2">
      <form
        ...
        class="flex items-center gap-2 p-1.5 pl-4 bg-gray-50 rounded-2xl border border-gray-100 focus-within:border-glow-primary-300 transition-colors"
      >
        <!-- Upload ảnh -->
        ...
        <button
          ...
          class="w-10 h-10 rounded-xl bg-gray-900 text-white flex items-center justify-center disabled:bg-gray-200 transition-all hover:bg-glow-primary-600"
        >
          <Send class="w-4 h-4" />
        </button>
      </form>
      <p class="text-[9px] text-center text-gray-400 font-medium uppercase tracking-tighter">
...
import { MessageCircle, Send, X, Bot, User, Maximize2, Image as ImageIcon } from 'lucide-vue-next';
...
const { messages, useSendMessage, useSendImage } = useChatbot();
...
const handleSend = () => {
  ...
  input.value = '';
  sendMsg(msg, { onSuccess: () => scrollToBottom() });
};

const handleImage = (e: Event) => {
  const file = (e.target as HTMLInputElement).files?.[0];
  if (file) sendImg(file, { onSuccess: () => scrollToBottom() });
};

watch(() => messages.value.length, () => setTimeout(scrollToBottom, 100));
watch(open, (val) => { if (val) setTimeout(scrollToBottom, 50); });
</script>
