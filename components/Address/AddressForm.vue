...
import { Home, Building2, MapPin,X } from 'lucide-vue-next'
...
const handleSubmit = async () => {
  ...
  const { full_name, phone, address } = formData.value
  ...
  try {
    emit('submit', {
      ...
      ward:         formData.value.ward || undefined,
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <div>
              ...
              <div class="flex gap-3">
                <button v-for="type in ADDRESS_TYPES" :key="type" type="button"
                  @click="formData.address_type = type"
                  class="flex-1 py-2 px-3 border rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2"
                  :class="formData.address_type === type
                    ? 'border-blue-500 bg-blue-50 text-blue-700'
                    : 'border-gray-300 text-gray-600 hover:border-gray-400'">
                  <component :is="typeIcon[type]" class="w-4 h-4" />
                  {{ type }}
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea v-model="formData.address" required rows="3"
                ...
                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" />
            </div>

            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Tỉnh/Thành</label>
                <input v-model="formData.city" type="text" placeholder="TP. Hồ Chí Minh"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Quận/Huyện</label>
                <input v-model="formData.district" type="text" placeholder="Quận 1"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Phường/Xã</label>
                <input v-model="formData.ward" type="text" placeholder="Phường Bến Nghé"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
            </div>

            <!-- Map Picker — giữ nguyên -->
            <MapPicker
            ...
            <div v-if="errorMessage"
              class="px-4 py-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl">
              {{ errorMessage }}
            </div>
            ...
            <div class="flex gap-3 pt-2 border-t border-gray-100">
              ...
              <button type="submit" :disabled="isSubmitting || !!phoneError"
                class="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
...
.modal-leave-active .bg-white { transition: transform 0.2s ease; }
.modal-enter-from .bg-white,
.modal-leave-to .bg-white { transform: scale(0.95) translateY(10px); }
</style>